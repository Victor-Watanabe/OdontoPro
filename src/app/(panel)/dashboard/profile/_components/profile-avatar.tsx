...
export function AvatarProfile({ avatarUrl, userId }: AvatarProfileProps){
    ...
    async function handleChange(e: ChangeEvent <HTMLInputElement>){
        if(e.target.files && e.target.files[0]){
            ...
            const newFillename = `${userId}`;
            const newFile = new File([image], newFillename, { type: image.type })
    ...
    return(
        <div className="relative w-40 h-40 md:w-48 md:h-48">
            <div className="relative flex items-center justify-center w-full h-full">
                <span className="absolute cursor-pointer z-2 bg-slate-50/80 rounded-full shadow-xl">
            ...
            {previewImage ? (
                ...
                quality={100}
                priority
                sizes="(max-width: 480) 100vw, (max-widht: 1024) 75vw, 60vw"/>
            ): (
                <Image
                src={semFoto}
                alt="Foto de perfil da clinica"
