...
export function SidebarDashoard({
  children,
}: {
  ...
  return (
    <div className="flex min-h-screen w-full">
      ...
      className={clsx("flex flex-col border-r bg-background transition-all duration-300 p-4 h-full",
        ...
        {isCollapsed && (
            <nav className="flex flex-col gap-1 overflow-hidden mt-02">
                <SidebarLink
                  href="/dashboard"
                  label="Agendamentos"
                  pathname={pathname}
      ...
      >
        ...
        <header className="md:hidden flex items-center justify-between border-b px-4 md:px-6 h-14 z-10 sticky top-0 bg-white">
          <Sheet>
            ...
            <div className="space-y-1 border-b pb-4 pt-4 pl-2">
  ...
  return (
    <Link href={href}>
      ...
      >
        ...
        {(forceOpen || !isCollapsed) && <span>{label}</span>}
      </div>
    </Link>
  );
}
