...
import { deleteReminder } from "../../_action/delete-reminder"
        ...
        <div className="flex flex-col gap-3">
            <Card>
                ...
                <CardContent>
                    ...
                    <ScrollArea className="h-85 lg:max-h-[calc(1--vh-15rem)] pr-0 w-full flex-1">
                        {reminder.map((item) => (
                            <article 
                            key={item.id}
                            className="flex flex-wrap flex-row items-center justify-between py-2 bg-yellow-100 mb-2 px-2 rounded-md">
                                <p className="text-sm lg:text-base">{item.description}</p>
                                <Button className="bg-red-500 hover:bg-red-400 shadow-none rounded-full p-2"
                                size="sm"
